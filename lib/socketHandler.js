...
function registerDevice(socket,req) {
  ...
  var partner = findWatcherByStream(req.stream)
  if (partner) {
    console.log('input device connected: ' + data);
    var device = new Device(req,socket.id);
    makePartners(device.id,partner.id);
    io.sockets.socket(partner.id).emit('data', {connection:true});
  } else {
...
function findWatcherByStream(stream) {
  ...
  for (var i=0; i < watchers.length; i++) {
    if (watchers[i].stream && watchers[i].stream === stream) {
      return watchers[i].id;
    } else {
      return false;
    }
  }
