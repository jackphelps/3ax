...
function asyncMakeStream(callback) {
    ...
    for (var i=0; i < chars.length; i++) {
      for (var j=0; j < chars.length; j++) {
        for (var k=0; k < chars.length; k++) {
          var str = chars[h] + chars[i] + chars[j] + chars[k];
          pushSetValue(set,str);
        }
      }
    }
  }
...
function pushSetValue(set,val) {
  redisClient.sadd('streamBuffer', val, function(e,r) {
  ...
  if (req.stream && req.cookie) {
    asyncLoad('Stream', req.stream, function(e,stream) {
      if (e || !validStreamRequest(req,stream)) {  
        asyncMakeStream(function(stream) {
